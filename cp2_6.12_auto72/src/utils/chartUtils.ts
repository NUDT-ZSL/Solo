import type { StyleMetrics, TrendDataPoint } from '@/types'

export interface RadarChartOptions {
  size?: number
  padding?: number
  animationDuration?: number
}

export interface LineChartOptions {
  width?: number
  height?: number
  padding?: {
    top?: number
    right?: number
    bottom?: number
    left?: number
  }
  animationDuration?: number
}

export const drawRadarChart = (
  ctx: CanvasRenderingContext2D,
  metrics: StyleMetrics,
  options: RadarChartOptions = {},
  progress: number = 1
): void => {
  const { size = 300, padding = 40 } = options
  const centerX = size / 2
  const centerY = size / 2
  const radius = (size - padding * 2) / 2

  const labels = ['暖色比例', '冷色比例', '饱和度', '明度', '对比度']
  const values = [
    metrics.warmRatio,
    metrics.coolRatio,
    metrics.saturation,
    metrics.brightness,
    metrics.contrast
  ]
  const angles = labels.map((_, i) => (-Math.PI / 2) + (i * 2 * Math.PI) / labels.length)

  ctx.clearRect(0, 0, size, size)

  for (let level = 5; level >= 1; level--) {
    const levelRadius = (radius * level) / 5
    ctx.beginPath()
    for (let i = 0; i < labels.length; i++) {
      const angle = angles[i]
      const x = centerX + levelRadius * Math.cos(angle)
      const y = centerY + levelRadius * Math.sin(angle)
      if (i === 0) {
        ctx.moveTo(x, y)
      } else {
        ctx.lineTo(x, y)
      }
    }
    ctx.closePath()
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.1)'
    ctx.lineWidth = 1
    ctx.stroke()
  }

  for (let i = 0; i < labels.length; i++) {
    const angle = angles[i]
    const x = centerX + radius * Math.cos(angle)
    const y = centerY + radius * Math.sin(angle)
    ctx.beginPath()
    ctx.moveTo(centerX, centerY)
    ctx.lineTo(x, y)
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.15)'
    ctx.lineWidth = 1
    ctx.stroke()
  }

  if (progress > 0) {
    const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, radius)
    gradient.addColorStop(0, 'rgba(103, 58, 183, 0.6)')
    gradient.addColorStop(1, 'rgba(103, 58, 183, 0.1)')

    ctx.beginPath()
    for (let i = 0; i < labels.length; i++) {
      const angle = angles[i]
      const value = (values[i] / 100) * radius * progress
      const x = centerX + value * Math.cos(angle)
      const y = centerY + value * Math.sin(angle)
      if (i === 0) {
        ctx.moveTo(x, y)
      } else {
        ctx.lineTo(x, y)
      }
    }
    ctx.closePath()
    ctx.fillStyle = gradient
    ctx.fill()
    ctx.strokeStyle = 'rgba(103, 58, 183, 0.8)'
    ctx.lineWidth = 2
    ctx.stroke()

    for (let i = 0; i < labels.length; i++) {
      const angle = angles[i]
      const value = (values[i] / 100) * radius * progress
      const x = centerX + value * Math.cos(angle)
      const y = centerY + value * Math.sin(angle)
      ctx.beginPath()
      ctx.arc(x, y, 4, 0, Math.PI * 2)
      ctx.fillStyle = '#673AB7'
      ctx.fill()
    }
  }

  for (let i = 0; i < labels.length; i++) {
    const angle = angles[i]
    const x = centerX + (radius + 20) * Math.cos(angle)
    const y = centerY + (radius + 20) * Math.sin(angle)
    ctx.fillStyle = '#333'
    ctx.font = '500 12px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText(labels[i], x, y)
  }
}

export const drawLineChart = (
  ctx: CanvasRenderingContext2D,
  data: TrendDataPoint[],
  options: LineChartOptions = {},
  progress: number = 1,
  hoverIndex: number | null = null
): { pointPositions: { x: number; y: number; data: TrendDataPoint }[] } => {
  const width = options.width ?? 600
  const height = options.height ?? 300
  const padding = {
    top: options.padding?.top ?? 40,
    right: options.padding?.right ?? 40,
    bottom: options.padding?.bottom ?? 50,
    left: options.padding?.left ?? 60
  }

  const chartWidth = width - padding.left - padding.right
  const chartHeight = height - padding.top - padding.bottom

  ctx.clearRect(0, 0, width, height)

  ctx.strokeStyle = 'rgba(0, 0, 0, 0.1)'
  ctx.lineWidth = 1
  for (let i = 0; i <= 4; i++) {
    const y = padding.top + (chartHeight * i) / 4
    ctx.beginPath()
    ctx.moveTo(padding.left, y)
    ctx.lineTo(width - padding.right, y)
    ctx.stroke()

    const value = Math.round(100 - (i * 100) / 4)
    ctx.fillStyle = '#666'
    ctx.font = '12px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
    ctx.textAlign = 'right'
    ctx.textBaseline = 'middle'
    ctx.fillText(value.toString(), padding.left - 10, y)
  }

  const pointPositions: { x: number; y: number; data: TrendDataPoint }[] = []

  if (data.length === 0) return { pointPositions: [] }

  const xStep = chartWidth / (data.length - 1)

  data.forEach((d, i) => {
    const x = padding.left + i * xStep
    const y = padding.top + chartHeight - (d.value / 100) * chartHeight
    pointPositions.push({ x, y, data: d })

    ctx.fillStyle = '#666'
    ctx.font = '12px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
    ctx.fillText(d.label, x, height - padding.bottom + 10)
  })

  if (progress > 0 && data.length > 1) {
    const visiblePoints = Math.ceil(data.length * progress)
    const remainder = (data.length * progress) % 1

    ctx.beginPath()
    ctx.moveTo(pointPositions[0].x, pointPositions[0].y)

    for (let i = 1; i < visiblePoints; i++) {
      const prev = pointPositions[i - 1]
      const curr = pointPositions[i]
      const cpx1 = prev.x + (curr.x - prev.x) / 3
      const cpx2 = prev.x + ((curr.x - prev.x) * 2) / 3
      const targetY = i === visiblePoints - 1 && remainder > 0 && i < data.length - 1
        ? prev.y + (curr.y - prev.y) * remainder
        : curr.y
      ctx.bezierCurveTo(cpx1, prev.y, cpx2, targetY, curr.x, targetY)
    }

    ctx.strokeStyle = '#673AB7'
    ctx.lineWidth = 2.5
    ctx.stroke()

    const visibleCount = Math.min(visiblePoints, data.length)
    for (let i = 0; i < visibleCount; i++) {
      const isLast = i === visibleCount - 1
      const point = pointPositions[i]
      const displayY = isLast && remainder > 0 && i < data.length - 1
        ? pointPositions[i - 1].y + (point.y - pointPositions[i - 1].y) * remainder
        : point.y

      ctx.beginPath()
      ctx.arc(point.x, displayY, hoverIndex === i ? 7 : 5, 0, Math.PI * 2)
      ctx.fillStyle = '#fff'
      ctx.fill()
      ctx.strokeStyle = '#673AB7'
      ctx.lineWidth = 2
      ctx.stroke()
    }
  }

  return { pointPositions }
}

export const hexToRgb = (hex: string): { r: number; g: number; b: number } | null => {
  const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex)
  return result
    ? {
        r: parseInt(result[1], 16),
        g: parseInt(result[2], 16),
        b: parseInt(result[3], 16)
      }
    : null
}

export const isWarmColor = (hex: string): boolean => {
  const rgb = hexToRgb(hex)
  if (!rgb) return false
  return rgb.r > rgb.b
}

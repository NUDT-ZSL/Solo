...
export default function PlayerDashboard() {
  ...
  async function loadStats() {
    if (!id) return;
    try {
      const data = await playerApi.getPlayerStats(id);
      setStats(data);
    } catch (error) {
  ...
  const barData = stats.recentScores.map((item, index) => ({
  ...
  return (
    <div className="dashboard-container">
      <h1 className="page-title">👤 {stats.playerName} 的数据中心</h1>
      ...
      <div className="card" style={{ marginTop: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
          <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 8 }}>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>总胜场</div>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#4caf50' }}>{stats.wins}</div>
          </div>
          <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 8 }}>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>总负场</div>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#f44336' }}>{stats.totalGames - stats.wins}</div>
          </div>
          <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 8 }}>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>游玩桌游数</div>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#64b5f6' }}>{stats.gameDistribution.length}</div>
          </div>
          <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 8 }}>
            <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>当前连胜</div>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#ff9800' }}>{stats.currentWinStreak}</div>
          </div>
        </div>
      </div>
    </div>

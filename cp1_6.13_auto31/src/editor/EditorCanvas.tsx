import { useRef, useEffect, useCallback, useState } from 'react'
import { useLevelStore } from '@/store/useLevelStore'
import {
  type LevelElement,
  type EnemyEntity,
  isEnemyElement,
  type ElementType,
  CANVAS_WIDTH,
  CANVAS_HEIGHT,
  GRID_SIZE,
  ELEMENT_DEFAULTS,
} from '@/types'

function snapToGrid(val: number): number {
  return Math.round(val / GRID_SIZE) * GRID_SIZE
}

function drawGrid(ctx: CanvasRenderingContext2D) {
  ctx.strokeStyle = '#2c2c34'
  ctx.lineWidth = 1
  for (let x = 0; x <= CANVAS_WIDTH; x += GRID_SIZE) {
    ctx.beginPath()
    ctx.moveTo(x + 0.5, 0)
    ctx.lineTo(x + 0.5, CANVAS_HEIGHT)
    ctx.stroke()
  }
  for (let y = 0; y <= CANVAS_HEIGHT; y += GRID_SIZE) {
    ctx.beginPath()
    ctx.moveTo(0, y + 0.5)
    ctx.lineTo(CANVAS_WIDTH, y + 0.5)
    ctx.stroke()
  }
}

function drawElement(ctx: CanvasRenderingContext2D, el: LevelElement) {
  switch (el.type) {
    case 'ground': {
      const grad = ctx.createLinearGradient(el.x, el.y, el.x, el.y + el.height)
      grad.addColorStop(0, '#4ade80')
      grad.addColorStop(1, '#22c55e')
      ctx.fillStyle = grad
      ctx.beginPath()
      ctx.roundRect(el.x, el.y, el.width, el.height, 3)
      ctx.fill()
      ctx.strokeStyle = '#16a34a'
      ctx.lineWidth = 1
      ctx.stroke()
      break
    }
    case 'movingPlatform': {
      const grad = ctx.createLinearGradient(el.x, el.y, el.x, el.y + el.height)
      grad.addColorStop(0, '#fb923c')
      grad.addColorStop(1, '#f97316')
      ctx.fillStyle = grad
      ctx.beginPath()
      ctx.roundRect(el.x, el.y, el.width, el.height, 3)
      ctx.fill()
      ctx.strokeStyle = '#ea580c'
      ctx.lineWidth = 1
      ctx.stroke()
      break
    }
    case 'spike': {
      ctx.fillStyle = '#dc2626'
      ctx.beginPath()
      ctx.moveTo(el.x, el.y + el.height)
      ctx.lineTo(el.x + el.width / 2, el.y)
      ctx.lineTo(el.x + el.width, el.y + el.height)
      ctx.closePath()
      ctx.fill()
      ctx.strokeStyle = '#991b1b'
      ctx.lineWidth = 1
      ctx.stroke()
      break
    }
    case 'flag': {
      ctx.fillStyle = '#92400e'
      ctx.fillRect(el.x + 2, el.y, 3, el.height)
      ctx.fillStyle = '#fbbf24'
      ctx.beginPath()
      ctx.moveTo(el.x + 5, el.y)
      ctx.lineTo(el.x + el.width, el.y + 8)
      ctx.lineTo(el.x + 5, el.y + 16)
      ctx.closePath()
      ctx.fill()
      ctx.strokeStyle = '#d97706'
      ctx.lineWidth = 1
      ctx.stroke()
      break
    }
    case 'slime': {
      const cx = el.x + el.width / 2
      const cy = el.y + el.height / 2
      const r = el.width / 2
      ctx.fillStyle = '#4ade80'
      ctx.beginPath()
      ctx.arc(cx, cy, r, 0, Math.PI * 2)
      ctx.fill()
      ctx.strokeStyle = '#16a34a'
      ctx.lineWidth = 1.5
      ctx.stroke()
      ctx.fillStyle = '#fff'
      ctx.beginPath()
      ctx.arc(cx - 5, cy - 3, 4, 0, Math.PI * 2)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(cx + 5, cy - 3, 4, 0, Math.PI * 2)
      ctx.fill()
      ctx.fillStyle = '#1a1a2e'
      ctx.beginPath()
      ctx.arc(cx - 4, cy - 3, 2, 0, Math.PI * 2)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(cx + 6, cy - 3, 2, 0, Math.PI * 2)
      ctx.fill()
      break
    }
    case 'dragon': {
      const cx = el.x + el.width / 2
      const cy = el.y + el.height / 2
      ctx.fillStyle = '#ef4444'
      ctx.beginPath()
      ctx.ellipse(cx, cy, el.width / 2, el.height / 2, 0, 0, Math.PI * 2)
      ctx.fill()
      ctx.strokeStyle = '#b91c1c'
      ctx.lineWidth = 1.5
      ctx.stroke()
      const wingOffset = Math.sin(Date.now() / 200) * 4
      ctx.fillStyle = '#f97316'
      ctx.beginPath()
      ctx.moveTo(cx - el.width / 2, cy)
      ctx.lineTo(cx - el.width / 2 - 10, cy - 8 + wingOffset)
      ctx.lineTo(cx - el.width / 2 + 4, cy - 4)
      ctx.closePath()
      ctx.fill()
      ctx.beginPath()
      ctx.moveTo(cx + el.width / 2, cy)
      ctx.lineTo(cx + el.width / 2 + 10, cy - 8 - wingOffset)
      ctx.lineTo(cx + el.width / 2 - 4, cy - 4)
      ctx.closePath()
      ctx.fill()
      ctx.fillStyle = '#fef08a'
      ctx.beginPath()
      ctx.arc(cx - 4, cy - 2, 2, 0, Math.PI * 2)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(cx + 4, cy - 2, 2, 0, Math.PI * 2)
      ctx.fill()
      break
    }
  }

  if (isEnemyElement(el)) {
    const enemy = el as EnemyEntity
    if (enemy.pathPoints && enemy.pathPoints.length > 1) {
      ctx.strokeStyle = 'rgba(96, 165, 250, 0.5)'
      ctx.lineWidth = 2
      ctx.setLineDash([5, 5])
      ctx.beginPath()
      const pts = enemy.pathPoints.map(p => ({
        x: p.x + enemy.width / 2,
        y: p.y + enemy.height / 2,
      }))
      ctx.moveTo(pts[0].x, pts[0].y)
      for (let i = 1; i < pts.length; i++) {
        ctx.lineTo(pts[i].x, pts[i].y)
      }
      ctx.stroke()
      ctx.setLineDash([])
      enemy.pathPoints.forEach(pt => {
        const px = pt.x + enemy.width / 2
        const py = pt.y + enemy.height / 2
        ctx.fillStyle = 'rgba(59, 130, 246, 0.2)'
        ctx.beginPath()
        ctx.arc(px, py, 9, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = '#93c5fd'
        ctx.beginPath()
        ctx.arc(px, py, 5, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = '#ffffff'
        ctx.beginPath()
        ctx.arc(px, py, 2, 0, Math.PI * 2)
        ctx.fill()
      })
    }
  }

  if (el.type === 'movingPlatform') {
    const mp = el as EnemyEntity & { pathPoints?: { x: number; y: number }[] }
    if (mp.pathPoints && mp.pathPoints.length > 1) {
      ctx.strokeStyle = 'rgba(96, 165, 250, 0.5)'
      ctx.lineWidth = 2
      ctx.setLineDash([5, 5])
      ctx.beginPath()
      const pts = mp.pathPoints.map(p => ({
        x: p.x + mp.width / 2,
        y: p.y + mp.height / 2,
      }))
      ctx.moveTo(pts[0].x, pts[0].y)
      for (let i = 1; i < pts.length; i++) {
        ctx.lineTo(pts[i].x, pts[i].y)
      }
      ctx.stroke()
      ctx.setLineDash([])
      mp.pathPoints.forEach(pt => {
        const px = pt.x + mp.width / 2
        const py = pt.y + mp.height / 2
        ctx.fillStyle = 'rgba(59, 130, 246, 0.2)'
        ctx.beginPath()
        ctx.arc(px, py, 9, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = '#93c5fd'
        ctx.beginPath()
        ctx.arc(px, py, 5, 0, Math.PI * 2)
        ctx.fill()
      })
    }
  }
}

function drawSelection(ctx: CanvasRenderingContext2D, el: LevelElement) {
  ctx.strokeStyle = '#60a5fa'
  ctx.lineWidth = 2
  ctx.setLineDash([5, 4])
  ctx.strokeRect(el.x - 4, el.y - 4, el.width + 8, el.height + 8)
  ctx.setLineDash([])
  ctx.strokeStyle = '#3b82f6'
  ctx.lineWidth = 1
  ctx.fillStyle = 'rgba(59, 130, 246, 0.12)'
  ctx.fillRect(el.x - 4, el.y - 4, el.width + 8, el.height + 8)
  const handleSize = 6
  const corners = [
    [el.x - 4, el.y - 4],
    [el.x + el.width - 2, el.y - 4],
    [el.x - 4, el.y + el.height - 2],
    [el.x + el.width - 2, el.y + el.height - 2],
  ]
  for (const [cx, cy] of corners) {
    ctx.fillStyle = '#ffffff'
    ctx.fillRect(cx - handleSize / 2, cy - handleSize / 2, handleSize, handleSize)
    ctx.strokeStyle = '#3b82f6'
    ctx.lineWidth = 1
    ctx.strokeRect(cx - handleSize / 2, cy - handleSize / 2, handleSize, handleSize)
  }
}

function drawPreview(
  ctx: CanvasRenderingContext2D,
  type: ElementType,
  x: number,
  y: number,
  valid: boolean,
) {
  const defaults = ELEMENT_DEFAULTS[type]
  const sx = snapToGrid(x)
  const sy = snapToGrid(y)
  ctx.globalAlpha = valid ? 0.55 : 0.35
  drawElement(ctx, {
    id: '__preview__',
    type,
    x: sx,
    y: sy,
    width: defaults.width,
    height: defaults.height,
  })
  ctx.globalAlpha = 1

  ctx.strokeStyle = valid ? '#60a5fa' : '#f87171'
  ctx.lineWidth = 2
  ctx.setLineDash([6, 4])
  ctx.strokeRect(sx - 2, sy - 2, defaults.width + 4, defaults.height + 4)
  ctx.setLineDash([])

  ctx.strokeStyle = valid ? 'rgba(96,165,250,0.8)' : 'rgba(248,113,113,0.8)'
  ctx.lineWidth = 1
  for (let gx = 0; gx <= CANVAS_WIDTH; gx += GRID_SIZE) {
    if (Math.abs(gx - (sx + defaults.width / 2)) < 1.5) {
      ctx.beginPath()
      ctx.moveTo(gx + 0.5, 0)
      ctx.lineTo(gx + 0.5, CANVAS_HEIGHT)
      ctx.stroke()
    }
  }
  for (let gy = 0; gy <= CANVAS_HEIGHT; gy += GRID_SIZE) {
    if (Math.abs(gy - (sy + defaults.height / 2)) < 1.5) {
      ctx.beginPath()
      ctx.moveTo(0, gy + 0.5)
      ctx.lineTo(CANVAS_WIDTH, gy + 0.5)
      ctx.stroke()
    }
  }

  ctx.fillStyle = valid ? '#60a5fa' : '#f87171'
  ctx.font = '12px ui-monospace, Menlo, monospace'
  ctx.fillText(`(${sx}, ${sy}) ${type}`, sx + defaults.width + 8, sy - 6)
}

interface PreviewState {
  type: ElementType
  x: number
  y: number
  valid: boolean
}

export default function EditorCanvas() {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const elements = useLevelStore(s => s.elements)
  const selectedId = useLevelStore(s => s.selectedId)
  const addElement = useLevelStore(s => s.addElement)
  const selectElement = useLevelStore(s => s.selectElement)
  const animFrameRef = useRef<number>(0)
  const [preview, setPreview] = useState<PreviewState | null>(null)
  const [isDragOver, setIsDragOver] = useState(false)

  const render = useCallback(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const ctx = canvas.getContext('2d')
    if (!ctx) return

    ctx.fillStyle = isDragOver ? '#23232c' : '#1e1e24'
    ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT)

    drawGrid(ctx)

    if (isDragOver) {
      ctx.fillStyle = 'rgba(59, 130, 246, 0.04)'
      ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT)
      ctx.strokeStyle = 'rgba(59, 130, 246, 0.5)'
      ctx.lineWidth = 2
      ctx.setLineDash([10, 6])
      ctx.strokeRect(3, 3, CANVAS_WIDTH - 6, CANVAS_HEIGHT - 6)
      ctx.setLineDash([])
    }

    elements.forEach(el => {
      drawElement(ctx, el)
    })

    const selected = elements.find(el => el.id === selectedId)
    if (selected) {
      drawSelection(ctx, selected)
    }

    if (preview) {
      drawPreview(ctx, preview.type, preview.x, preview.y, preview.valid)
    }

    const hasDragon = elements.some(el => el.type === 'dragon')
    if (hasDragon || isDragOver) {
      animFrameRef.current = requestAnimationFrame(render)
    }
  }, [elements, selectedId, preview, isDragOver])

  useEffect(() => {
    render()
    return () => {
      if (animFrameRef.current) {
        cancelAnimationFrame(animFrameRef.current)
      }
    }
  }, [render])

  const getCanvasCoords = (clientX: number, clientY: number): { x: number; y: number } => {
    const canvas = canvasRef.current
    if (!canvas) return { x: 0, y: 0 }
    const rect = canvas.getBoundingClientRect()
    const scaleX = CANVAS_WIDTH / rect.width
    const scaleY = CANVAS_HEIGHT / rect.height
    return {
      x: (clientX - rect.left) * scaleX,
      y: (clientY - rect.top) * scaleY,
    }
  }

  const parseDropType = (dataTransfer: DataTransfer): ElementType | null => {
    const types: ElementType[] = ['ground', 'movingPlatform', 'spike', 'flag', 'slime', 'dragon']
    let t = dataTransfer.getData('application/x-levelforge-type')
    if (!t) t = dataTransfer.getData('element-type')
    if (!t) t = dataTransfer.getData('text/plain')
    const cleaned = (t || '').trim().toLowerCase() as ElementType
    return types.includes(cleaned) ? cleaned : null
  }

  const handleDragEnter = (e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()
    const dt = parseDropType(e.dataTransfer)
    if (dt) {
      setIsDragOver(true)
    }
  }

  const handleDragLeave = (e: React.DragEvent) => {
    e.preventDefault()
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
    const x = e.clientX
    const y = e.clientY
    if (x <= rect.left || x >= rect.right || y <= rect.top || y >= rect.bottom) {
      setIsDragOver(false)
      setPreview(null)
    }
  }

  const handleDragOver = (e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()
    e.dataTransfer.dropEffect = 'copy'
    e.dataTransfer.effectAllowed = 'copy'

    const dt = parseDropType(e.dataTransfer)
    if (!dt) return

    const { x, y } = getCanvasCoords(e.clientX, e.clientY)
    const defaults = ELEMENT_DEFAULTS[dt]
    const sx = snapToGrid(x)
    const sy = snapToGrid(y)
    const valid =
      sx >= 0 &&
      sy >= 0 &&
      sx + defaults.width <= CANVAS_WIDTH &&
      sy + defaults.height <= CANVAS_HEIGHT

    setPreview({ type: dt, x, y, valid })
  }

  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()

    const dt = parseDropType(e.dataTransfer)
    if (!dt) {
      setIsDragOver(false)
      setPreview(null)
      return
    }

    const { x, y } = getCanvasCoords(e.clientX, e.clientY)
    const defaults = ELEMENT_DEFAULTS[dt]
    const sx = snapToGrid(x)
    const sy = snapToGrid(y)

    if (sx >= 0 && sy >= 0 && sx + defaults.width <= CANVAS_WIDTH && sy + defaults.height <= CANVAS_HEIGHT) {
      addElement(dt, sx + defaults.width / 2, sy + defaults.height / 2)
    }

    setIsDragOver(false)
    setPreview(null)
  }

  const handleClick = (e: React.MouseEvent) => {
    const { x: mx, y: my } = getCanvasCoords(e.clientX, e.clientY)

    let found: LevelElement | null = null
    for (let i = elements.length - 1; i >= 0; i--) {
      const el = elements[i]
      if (mx >= el.x && mx <= el.x + el.width && my >= el.y && my <= el.y + el.height) {
        found = el
        break
      }
    }

    selectElement(found ? found.id : null)
  }

  return (
    <canvas
      ref={canvasRef}
      width={CANVAS_WIDTH}
      height={CANVAS_HEIGHT}
      onDrop={handleDrop}
      onDragOver={handleDragOver}
      onDragEnter={handleDragEnter}
      onDragLeave={handleDragLeave}
      onClick={handleClick}
      style={{
        borderRadius: 8,
        cursor: isDragOver ? 'copy' : 'pointer',
        maxWidth: '100%',
        height: 'auto',
        transition: 'all 0.2s',
        boxShadow: isDragOver ? '0 0 0 2px rgba(59,130,246,0.5)' : '0 4px 16px rgba(0,0,0,0.2)',
      }}
    />
  )
}

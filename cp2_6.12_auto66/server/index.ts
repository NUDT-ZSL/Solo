...
app.get('/api/proposals', (req, res) => {
  try {
    ...
    const proposals = getAllProposals(sessionId);
...
app.post('/api/vote', (req, res) => {
  try {
    ...
    const result = handleVote(proposalId, voteType, sessionId);
    res.json(result);
  } catch (error: any) {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', message: '音乐投票服务运行中' });
});

app.listen(PORT, () => {
  console.log(`🚀 服务器运行在 http://localhost:${PORT}`);
  console.log(`📡 API 端点: /api/proposals, /api/vote, /api/results`);
});

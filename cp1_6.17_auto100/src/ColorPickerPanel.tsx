...
import { buildGradientCSS, generateId, randomColor } from './types';
...
export default function ColorPickerPanel({
  ...
  const handleAddNode = useCallback(() => {
    ...
    const positions = colorNodes.map((n) => n.position);
    ...
    const newNode: ColorNode = {
      id: generateId(),
      color: randomColor(),
      position: Math.min(100, avgPos + 10),
    };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        >
          {isLinear ? '角度' : '半径'}
          <span style={{ marginLeft: 8, color: '#d4d4d4' }}>
            {angle}{isLinear ? '°' : '%'}
          </span>
        </label>
        <input

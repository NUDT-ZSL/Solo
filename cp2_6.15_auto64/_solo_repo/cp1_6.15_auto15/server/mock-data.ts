...
export const KNOWLEDGE_TAGS: string[] = [
  'JavaScript基础', 'TypeScript进阶', 'React框架', 'Node.js服务端',
  'CSS布局与动画', 'HTTP与网络', '数据结构与算法', '设计模式',
  '数据库基础', '安全与性能优化',
];

type CoreQ = { q: string; opts: [string, string, string, string]; ans: number; exp: string; tag: string; diff: 1|2|3 };
...
function expandQuestions(core: CoreQ[], targetPerTag: number): Question[] {
  const result: Question[] = [];
  const byTag: Record<string, CoreQ[]> = {};
  core.forEach(c => {
  ...
  KNOWLEDGE_TAGS.forEach(tag => {
    ...
    for (let i = 0; i < targetPerTag; i++) {
      ...
      const optsShuffled = shuffle([...base.opts] as [string,string,string,string], i + targetPerTag + tag.length);
      ...
      result.push({
        id: uuidv4(),
        question: qText,
        options: optsShuffled as [string, string, string, string],
...
export const QUESTION_BANK: Question[] = expandQuestions(CORE, 30);
...
export function getBalancedQuestions(count: number): Question[] {
  const perTag = Math.ceil(count / KNOWLEDGE_TAGS.length);
  const perDiff = Math.ceil(count / 3);
  const questions: Question[] = [];
  const usedIds = new Set<string>();
  const tagsShuffled = shuffle(KNOWLEDGE_TAGS, Date.now() % 10000);
  const diffs: (1|2|3)[] = [1, 2, 3];
  for (const tag of tagsShuffled) {
    for (const diff of diffs) {
      const pool = QUESTION_BANK.filter(q =>
        q.knowledgeTag === tag && q.difficulty === diff && !usedIds.has(q.id)
      );
      const shuffled = shuffle(pool, (tag.length + diff) * 17);
      for (let i = 0; i < Math.ceil(perDiff / KNOWLEDGE_TAGS.length) && i < shuffled.length; i++) {
  ...
  while (questions.length < count) {
    const remain = QUESTION_BANK.filter(q => !usedIds.has(q.id));
...
export function getWeakPointTags(ids: string[]): string[] {
  const set = new Set(ids);
  const wrong = QUESTION_BANK.filter(q => set.has(q.id));

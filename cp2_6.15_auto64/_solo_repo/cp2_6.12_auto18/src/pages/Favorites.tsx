...
export default function Favorites() {
  const { polls, favorites, fetchPolls, fetchFavorites, toggleFavorite, isLoggedIn, setShowLoginModal } = useStore();
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="text-center text-gray-400 py-20">
          ...
          <p className="text-sm mt-1">登录后即可查看你收藏的投票</p>
          <button
            onClick={() => setShowLoginModal(true)}
            className="inline-block mt-4 btn-interactive bg-gradient-to-r from-[#4facfe] to-[#00f2fe] text-white px-5 py-2 rounded-lg text-sm font-medium"
          >
            立即登录
          </button>
        </div>

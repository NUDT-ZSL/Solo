...
const ParticleBackground = ({ color }: ParticleBackgroundProps) => {
  ...
  useEffect(() => {
    ...
    const createParticle = (): Particle => {
      ...
      return {
        ...
        color: `rgba(${rgb.r + Math.random() * 50}, ${rgb.g + Math.random() * 50}, ${rgb.b + Math.random() * 50}, `
      }
    }
    ...
    const animate = (timestamp: number) => {
      ...
      if (timestamp - lastTimeRef.current < interval / 2) {
        return
      }
  ...
  return (
    <canvas
      ...
      style={{ background: 'linear-gradient(135deg, #1a0033 0%, #0d0221 100%)' }}
    />
  )
}

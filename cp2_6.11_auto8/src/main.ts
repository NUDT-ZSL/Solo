...
class Game {
  ...
  private resultShockwaves: ResultShockwave[] = [];
  ...
  private showResult(combo: ComboInfo): void {
    ...
    this.resultShockwaves = [
      { radius: 0, alpha: 1, startDelay: 0, elapsed: 0, started: false },
      { radius: 0, alpha: 1, startDelay: 0.12, elapsed: 0, started: false },
      { radius: 0, alpha: 1, startDelay: 0.24, elapsed: 0, started: false },
    ];
  ...
  private update(deltaTime: number): void {
    ...
    this.resultShockwaves.forEach(wave => {
      ...
      if (!wave.started && wave.elapsed >= wave.startDelay) {
        wave.started = true;
      }
      if (wave.started) {
        wave.radius += 500 * deltaTime;
        wave.alpha -= deltaTime * 2.0;
  ...
  private renderStatsChart(): void {
    ...
    if (this.statsCanvas.width !== Math.round(rect.width * dpr)) {
      this.statsCanvas.width = Math.round(rect.width * dpr);
      this.statsCanvas.height = Math.round(rect.height * dpr);

...
async function startServer() {
  ...
  if (db.data.books.length === 0) {
    db.data.books = seedBooks;
    db.data.chapters = seedChapters;
    db.data.annotations = seedAnnotations;
    db.data.comments = seedComments;
    db.data.likes = [];
    await db.write();
  ...
  app.get('/api/chapters/:bookId', (req, res) => {
    const chapters = db.data.chapters
      .filter(c => c.book_id === req.params.bookId)
      .sort((a, b) => a.chapter_order - b.chapter_order)
      .map(({ id, title, chapter_order, claimed_by }) => ({ id, title, chapter_order, claimed_by }));
    res.json(chapters);
  });

  app.get('/api/chapter/:id', (req, res) => {
    const chapter = db.data.chapters.find(c => c.id === req.params.id);
    if (!chapter) return res.status(404).json({ error: 'Chapter not found' });
    const annotations = db.data.annotations
      .filter(a => a.chapter_id === req.params.id)
      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    res.json({ ...chapter, annotations });
  });

  app.post('/api/claim-chapter', async (req, res) => {
  ...
  app.post('/api/annotations', async (req, res) => {
    ...
    const annotation: Annotation = {
      ...
      highlight_color: highlightColor || '#fff59d',
      body,
      likes: 0,
      created_at: now,
    };
  ...
  app.get('/api/annotations/like/status', (req, res) => {
    const { annotationId, userId } = req.query;
    const existing = db.data.likes.find(l => l.annotation_id === annotationId && l.user_id === userId);
    res.json({ liked: !!existing });
  });

  app.post('/api/annotations/like', async (req, res) => {
    const { annotationId, userId } = req.body;
    const existingIndex = db.data.likes.findIndex(l => l.annotation_id === annotationId && l.user_id === userId);
    const annotation = db.data.annotations.find(a => a.id === annotationId);
    
    if (!annotation) return res.status(404).json({ error: 'Annotation not found' });

    if (existingIndex !== -1) {
      db.data.likes.splice(existingIndex, 1);
      annotation.likes = Math.max(0, annotation.likes - 1);
      await db.write();
  ...
  app.get('/api/comments/:annotationId', (req, res) => {
    const comments = db.data.comments
      .filter(c => c.annotation_id === req.params.annotationId)

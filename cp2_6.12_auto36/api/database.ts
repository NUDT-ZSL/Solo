...
function seedData(database: Database) {
  ...
  database.run(
    ...
    [uuidv4(), doc1Id, database.exec(`SELECT content FROM documents WHERE id = ?`, [doc1Id])[0]?.values[0][0] as string, 1]
  )
  database.run(
    `INSERT INTO document_versions (id, document_id, content, version_number) VALUES (?, ?, ?, ?);`,
    [uuidv4(), doc2Id, database.exec(`SELECT content FROM documents WHERE id = ?`, [doc2Id])[0]?.values[0][0] as string, 1]
  )
  database.run(
    `INSERT INTO document_versions (id, document_id, content, version_number) VALUES (?, ?, ?, ?);`,
    [uuidv4(), doc3Id, database.exec(`SELECT content FROM documents WHERE id = ?`, [doc3Id])[0]?.values[0][0] as string, 1]
  )

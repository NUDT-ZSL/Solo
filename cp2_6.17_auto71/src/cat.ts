...
export class Cat {
  ...
  updateAnimations(deltaTime: number): void {
    ...
    this.displayHealth = this.lerp(this.displayHealth, this.health, deltaTime);
    this.displayHunger = this.lerp(this.displayHunger, this.hunger, deltaTime);
    this.displayHappiness = this.lerp(this.displayHappiness, this.happiness, deltaTime);
  }

  private lerp(current: number, target: number, deltaTime: number): number {
    const speed = 0.008;

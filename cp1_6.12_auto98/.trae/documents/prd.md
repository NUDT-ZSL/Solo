...
| 页面名称 | 模块名称 | 功能描述 |
|----------|----------|----------|
| 3D路径场景页 | 3D路径渲染 | Catmull-Rom样条插值生成平滑路径（插值步长0.5单位），海拔渐变着色（青→绿→黄→红，每0.5单位海拔平滑过渡），路径线宽0.15单位 |
| 3D路径场景页 | 地形曲面 | 路径下方带状地形网格（网格分辨率0.1单位），等高线纹理（每0.2单位海拔一条，线宽0.005单位），边缘线性缓坡过渡到0 |
| 3D路径场景页 | 相机动画 | 8秒环绕飞行（绕Y轴360度），高度在路径最高点上下浮动20%，动画结束后启用OrbitControls（阻尼0.1） |
| 侧边栏面板 | 文件上传 | CSV文件拖拽/点击上传，支持lat,lng,ele格式，拖拽时虚线边筐变实线金色，箭头旋转10度（300ms） |
| 侧边栏面板 | 手动输入 | 支持至少5个坐标点手动输入，格式lat,lng,ele |
| 侧边栏面板 | 统计信息 | 路径总距离(km，两位小数)和累计爬升(m，整数) |
| 侧边栏面板 | 海拔剖面 | Chart.js折线图，横轴距离百分比，纵轴海拔，折线渐变色与路径一致，半透明蓝色填充 |

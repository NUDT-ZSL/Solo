...
const Schedule: React.FC<ScheduleProps> = ({ onToast, onBottomNotify }) => {
  ...
  const handleSaveCourse = async (data: Partial<Course> & { name: string; duration: number; teacher: string; maxStudents: number; startTime: string }) => {
    try {
      if (editingCourse) {
        const res = await http.put(`/courses/${editingCourse.id}`, data)
      ...
      if (err.data?.conflict) {
        ...
        if (ok && editingCourse) {
          try {
            await http.put(`/courses/${editingCourse.id}`, { ...data, force: true })
  ...
  const handleSubmitReschedule = async (originalTime: string, newTime: string, remark: string) => {
    ...
    try {
      await http.post('/reschedule', {
        courseId: rescheduleCourse.id,
        originalTime,
        newTime,
        remark
      })
  ...
  const handleDragStart = (e: React.MouseEvent, course: Course) => {
    ...
    e.preventDefault()
    setDragging({ course, startY: e.clientY, offset: 0 })

    const onMove = (ev: MouseEvent) => {
      const diff = ev.clientY - (dragging?.startY ?? e.clientY)
    ...
    const onUp = async (ev: MouseEvent) => {
      ...
      const hourBlocks = Math.round(dragging.offset / 60)
      if (hourBlocks === 0) {
        setDragging(null)
        return
      }

      const oldDate = new Date(dragging.course.startTime)
      const newDate = new Date(oldDate)
      newDate.setHours(newDate.getHours() + hourBlocks)

      try {
        const res = await http.put(`/courses/${dragging.course.id}`, { startTime: newDate.toISOString() })
        onBottomNotify(`已调整「${dragging.course.name}」时间`)
        await fetchCourses()
      } catch (err: any) {
        if (err.data?.conflict) {
          ...
          if (ok) {
            try {
              await http.put(`/courses/${dragging.course.id}`, { startTime: newDate.toISOString(), force: true })
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '60px repeat(7, 1fr)', position: 'relative' }}>
          ...
          {weekDates.map((d, dayIdx) => (
            <div key={dayIdx} style={{
              position: 'relative',
              borderLeft: dayIdx === 0 ? 'none' : '1px solid #ecf0f1',
              borderTop: '1px solid #f4f6f7'
            }}>
              {HOURS.map(h => (
  ...
  const roleTabs = (
    <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
      {(['student', 'teacher', 'admin'] as Role[]).map(r => (
        ...
        >
          {r === 'student' ? '👤 学员视角' : r === 'teacher' ? '👨‍🏫 教师视角' : '⚙️ 管理员视角'}
        </button>
      ))}
    </div>
  )
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button
          ...
          style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #dfe4ea', background: '#fff', cursor: 'pointer', fontSize: 13 }}
        >← 上周</button>
        <button
          onClick={() => setWeekOffset(0)}
          style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #3498db', background: '#3498db', color: '#fff', cursor: 'pointer', fontSize: 13 }}
        >本周</button>
        <button
          onClick={() => setWeekOffset(w => w + 1)}
          style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #dfe4ea', background: '#fff', cursor: 'pointer', fontSize: 13 }}
  ...
  if (role === 'teacher') {
    return (
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 80px)', margin: -24, padding: 0 }}>
        ...
        {showRescheduleModal && rescheduleCourse && (
          <RescheduleModal
            course={rescheduleCourse}
            onClose={() => { setShowRescheduleModal(false); setRescheduleCourse(null) }}
      ...
      {role === 'admin' && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            {courses.length === 0 && (
              <div style={{ padding: 40, color: '#95a5a6', fontSize: 13 }}>暂无课程，点击「新建课程」开始排课</div>
            )}
          </div>
        </div>
      )}
...
interface CDMProps {
  ...
  onDrop: (c: Course) => void
  onEdit: (c: Course) => void
  onDelete: (c: Course) => void
  onRequestReschedule: (c: Course) => void
  isEnrolled: boolean
}
    ...
    <ModalOverlay onClose={onClose}>
      <div style={{ background: '#fff', borderRadius: 12, width: 420, maxWidth: '90vw', maxHeight: '90vh', overflow: 'auto', padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            <div style={{ display: 'inline-block', background: status.color, color: '#fff', fontSize: 12, padding: '4px 10px', borderRadius: 8, marginBottom: 8 }}>
              {status.label}
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#2c3e50' }}>{course.name}</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#95a5a6' }}>×</button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 10, fontSize: 13, marginBottom: 20 }}>
          <div style={{ color: '#7f8c8d' }}>授课教师</div>
          <div style={{ color: '#2c3e50', fontWeight: 500 }}>{course.teacher}</div>
          <div style={{ color: '#7f8c8d' }}>课程时长</div>
          <div style={{ color: '#2c3e50', fontWeight: 500 }}>{course.duration} 分钟</div>
          <div style={{ color: '#7f8c8d' }}>上课时间</div>
          <div style={{ color: '#2c3e50', fontWeight: 500 }}>{formatDateTime(course.startTime)}</div>
          ...
          {role === 'student' && (
            isEnrolled ? (
              ...
              <button
                ...
                disabled={course.status === 'ended' || course.enrolledStudents >= course.maxStudents}
                style={{
                ...
                onMouseEnter={(e) => { if (e.currentTarget.style.background === '#3498db') e.currentTarget.style.background = '#2980b9' }}
                onMouseLeave={(e) => { if (!e.currentTarget.disabled) e.currentTarget.style.background = course.status === 'ended' || course.enrolledStudents >= course.maxStudents ? '#95a5a6' : '#3498db' }}
              >
    ...
    <ModalOverlay onClose={onClose}>
      <div style={{ background: '#fff', borderRadius: 12, width: 440, maxWidth: '90vw', padding: 24, maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
          ...
          <button
            onClick={() => {
              if (!form.name || !form.teacher || !form.startTime || !form.duration || !form.maxStudents) {
                alert('请填写所有必填字段')
                return
              }
              const startTimeIso = new Date(form.startTime).toISOString()
              onSave({ ...form, startTime: startTimeIso })
            }}
            style={{ flex: 1, padding: '10px 16px', borderRadius: 8, border: 'none', background: '#3498db', color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}
          >保存</button>
        </div>
      </div>
    </ModalOverlay>
...
interface RMProps {
  course: Course
  onClose: () => void
  onSubmit: (originalTime: string, newTime: string, remark: string) => void
}

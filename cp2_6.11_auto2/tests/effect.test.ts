...
function runEffectTests(): void {
  ...
  assert(updatedParticles.every((p, i) => p.x !== initialParticles[i].x || p.y !== initialParticles[i].y),
    '粒子位置发生变化');
  ...
  effectSys.update(150);
  assert(effectSys.getBoardOpacity() > 0 && effectSys.getBoardOpacity() < 1,
    '淡入中透明度在0-1之间');
  effectSys.update(200);

...
function createCustomIcon(color: string, isActive: boolean) {
  return L.divIcon({
    className: 'custom-marker',
    html: `<div class="memory-pin ${isActive ? 'active' : ''}" style="background-color: ${color};"></div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 32],
    popupAnchor: [0, -32]
  });
      ...
      >
        ...
        {memories.map((memory) => (
          <Marker
            ...
            icon={createCustomIcon(MOOD_COLOR[memory.mood], highlightedMemoryId === memory.id)}
            eventHandlers={{
              click: () => {
          ...
          >
            <Popup>
              <div style={{ minWidth: '200px' }}>
                <h3 style={{ margin: '0 0 8px 0', fontSize: '16px', color: '#8B4513' }}>
                  {memory.title}
                </h3>
